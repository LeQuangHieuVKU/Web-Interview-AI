...
function Step3Report({ report }) {
  ...
  const questionWiseScore = Array.isArray(report.questionWiseScore)
    ...
    : Array.isArray(report.questionsWiseScore)
      ? report.questionsWiseScore
      : [];

  const derivedFinalScore = questionWiseScore.length
    ? questionWiseScore.reduce((sum, q) => sum + (Number(q.score) || 0), 0) /
      questionWiseScore.length
    : 0;

  const safeFinalScore =
    Number(finalScore) > 0 ? Number(finalScore) : derivedFinalScore;
  ...
  const score = Number(safeFinalScore.toFixed(1));
  ...
  const downloadPDF = () => {
    ...
    //Final Score Box
    doc.setFillColor(240, 153, 244);
    doc.roundedRect(margin, currentY, contentWidth, 20, 4, 4, "F");

    doc.setFontSize(14);
    doc.setTextColor(0, 0, 0);
    doc.text(`Final Score: ${score}/10`, pageWidth / 2, currentY + 12, {
      align: "center",
    });
    ...
    if (score >= 8) {
      advice =
        "Excellent performance! Maintain confidence and structure. Continue refining clarity and supporting answers with strong real-world examples. You're well-prepared for job opportunities!";
    } else if (score >= 5) {
      advice =
        "Good foundation shown. Improve clarity and structure.Practice answsers aloud regularly";
    } else {
      advice =
        "Significant improvement needed. Focus on clarity, structure, and confidence. Practice articulating answers aloud, seek feedback, and consider mock interviews to build skills.";
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="space-y-6">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 text-center">
            <h3 className="text-gray-500 mg-4 sm:mb-6 text-sm sm:text-base">
              Overall Performance
            </h3>
        ...
        <div className="lg:col-span-2 space-y-6 min-w-0">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-5 sm:p-8 min-w-0">
